/*
 * read.c:
 *	Very simple program to read voltage
 *
 */
...
void ADS1256_StartScan(uint8_t _ucScanMode, uint8_t _chnum);
   ...
   printf("ID=\r\n");  
	...
	//if (ADS1256_Scan() == 0)
	   ...
	   if (ADS1256_Scan() == 0)
	   {
		   ...
		   result = iTemp / 1000000.0;
		   FILE *fptr;
		   fptr = fopen("result.txt", "w");
		   fprintf(fptr, "%0.9f", result);
		   fclose(fptr);
		   printf("%.9f \r\n", result);
	   }
/*		   if (iTemp < 0)
